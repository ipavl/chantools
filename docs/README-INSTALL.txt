...
============
 ...
 1. After downloading the .jar file, make sure PircBot.jar is present in /lib OR in the IRCBot.jar file (in org.jibble.pircbot).
 	a) If not, download it from the website of this bot <coming eventually>, or alternatively at <www.jibble.org>.
		i) If you downloaded it, make sure PircBot.jar is in /lib.
 2. Open a command prompt window and enter these commands (this can be automated with a batch file):
 	...
 	b) java -jar IRCBot.jar (where IRCBot.jar is the name of the bot you downloaded originally, NOT PircBot.jar)
 3. On first startup, you should receive a notice telling you to edit the /data/bot.properties file. Do so via a text editor (e.g. Notepad)
